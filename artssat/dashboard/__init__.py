...
class ProfilePlot:
    ...
    def __init__(self, z, p, title, name, width=500):
        ...
        self.x_axis_scale = RadioButtonGroup(labels=["linear", "log"], active=0)
        self.x_axis_scale.on_change("active", update_x_scale)

        self.y_axis_scale = RadioButtonGroup(labels=["linear", "log"], active=0)
    ...
    def __init__(self, z, p, retrieval):
        ...
        self.figures = dict(
            [
                (
                    rq,
                    ProfilePlot(
                        self.z, self.p, rq.name, "rq_plot_" + rq.name, width=400
                    ),
                )
        ...
        for i, rr in enumerate(self.retrieval.results):
            for rq in self.retrieval_quantities:
                ...
                fig = self.figures[rq]
                fig.add_quantity(
                    x, line_kwargs=dict(line_color=self.colors[i], line_width=2)
                )
                if i == 0:
                    fig.add_quantity(
                        xa,
                        line_kwargs=dict(
                            line_dash="dashed", line_color=self.colors[i], line_width=2
                        ),
                    )
        ...
        self.plots = row(
            *[self.figures[k].doc for k in self.figures], name="retrieval_quantities"
        )
...
class AVKPlot:
    ...
    def _make_avk_plot(self, result, rq):
        ...
        self.sources[rq] = ColumnDataSource(
            data=dict(x=xx.ravel(), y=yy.ravel(), z=avk.ravel())
        )
        self.figures[rq] = figure(
            x_range=(x[0] - 0.5, x[-1] + 0.5),
        ...
        mapper = LinearColorMapper(palette=colors, low=clim_low, high=clim_high)
        self.images[rq] = self.figures[rq].rect(
            source=self.sources[rq],
            x="x",
            y="y",
...
class ObservationPlot:
    ...
    def make_active_sensor_plot(self):
        ...
        fig.xaxis.axis_label = "Radar reflectivity [{0}]".format(self.sensor.iy_unit)
...
def make_atmosphere_panel(simulation, data_provider):
    c = column(
        plot_temperature(simulation, data_provider),
...
def make_sensor_plot(sensor):
    ...
    if isinstance(sensor, ActiveSensor):
        ...
        fig.xaxis.axis_label = "Radar reflectivity [{0}]".format(sensor.iy_unit)
        fig.yaxis.axis_label = "Altitude [km]"
...
def make_sensor_panel(sensors):
    c = column(*[make_sensor_plot(s) for s in sensors], sizing_mode="fixed")
    ...
    if type(simulation.retrieval.results) is list:
        ...
        for r in simulation.retrieval.results:
            ...
            for q in r.retrieval_quantities:
                x = r.get_result(q)
                plot.add_quantity(x, r.name, dict(line_dash="solid", line_width=3))
...
def make_document_factory(simulation):
    def make_document(doc):
        ...
        t = Tabs(tabs=[sensor_p, atmosphere_p, retrieval_p])
        doc.add_root(t)
